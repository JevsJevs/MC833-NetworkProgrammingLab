...
int main(int argc, char *argv[]) {
    ...
    memset(&client_sockets, -1, sizeof(client_sockets)); //seta invalidos todos os clientes por hora
    // memset(&master_set, 1, sizeof(client_sockets)); //seta invalidos todos os clientes por hora

    FD_ZERO(&master_set);
    FD_SET(listenfd, &master_set); //Adiciona o fd escutando conexoes ao set
    
    // laço: aceita clientes, envia banner e fecha a conexão do cliente
    int loopCount = 0;
    for (;;) {
        read_set = master_set; // copia master_set para o read_set

        // printf("loopAtual: %d\n", loopCount);
        (void) Select(maxfd + 1, &read_set, NULL, NULL, NULL);

        if (FD_ISSET(listenfd, &read_set)) { //Detectada nova conexão ao ter escrita no fd do Listen
        ...
        for (int i = 0; i < MAX_CLIENTS; i++) {
            ...
            if (fd < 0) //fd não alocado -> passa
                continue;
            printf("Dentro Loop CLient: FD: %d\n", fd);
            if (FD_ISSET(fd, &read_set)) { // -> se fd pronto para ler
